...
static char batch_kquery_docstring[] = "Method to get k nearest neighbors in a batch of stacked pointclouds";
...
static PyObject *batch_kneighbors(PyObject *self, PyObject *args, PyObject *keywds);
...
static PyMethodDef module_methods[] = 
{
	...
	{ "batch_kquery", (PyCFunction)batch_kneighbors, METH_VARARGS | METH_KEYWORDS, batch_kquery_docstring },
	{NULL, NULL, 0, NULL}
};
...
static PyObject* batch_kneighbors(PyObject* self, PyObject* args, PyObject* keywds)
{
	...
	// Keywords containers
	static char* kwlist[] = { "queries", "supports", "q_batches", "s_batches", "K", NULL };
	int K = 16;

	// Parse the input  
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "OOOO|i", kwlist, &queries_obj, &supports_obj, &q_batches_obj, &s_batches_obj, &K))
	{
		PyErr_SetString(PyExc_RuntimeError, "Error parsing arguments");
		return NULL;
	}
	...
	// Compute results
	//batch_ordered_neighbors(queries, supports, q_batches, s_batches, neighbors_indices, radius);
	batch_nanoflann_knn(queries, supports, q_batches, s_batches, neighbors_indices, K);
	...
	// Create output array
	PyObject* res_obj = PyArray_SimpleNew(2, neighbors_dims, NPY_ULONG);
	PyObject* ret = NULL;

	// Fill output array with values
	size_t size_in_bytes = Nq * K * sizeof(size_t);
	memcpy(PyArray_DATA(res_obj), neighbors_indices.data(), size_in_bytes);
