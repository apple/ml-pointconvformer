                                ...
                                float radius)
{
	...
	int max_count = 0;
	float d2;
	vector<vector<nanoflann::ResultItem<unsigned int, float>>> all_inds_dists(queries.size());
...
void batch_nanoflann_knn(vector<PointXYZ>& queries,
                                vector<PointXYZ>& supports,
                                vector<int>& q_batches,
                                vector<int>& s_batches,
                                vector<unsigned int>& neighbors_indices,
                                const size_t K)
{
	...
	// indices
	int i0 = 0;
/*#	vector<vector<size_t>> all_inds_dists(queries.size()); */
    ...
    nanoflann::SearchParameters search_params;
    search_params.sorted = true;
    neighbors_indices.resize(queries.size() * K);
...
//        all_inds_dists[i0].reserve(K);

	    // Find neighbors
	    float query_pt[3] = { p0.x, p0.y, p0.z};
	    vector<float> out_dist_sqr;
	    out_dist_sqr.reserve(K);
	    index->knnSearch(query_pt, K, &neighbors_indices[i0*K], &out_dist_sqr[0]);
	...
	for (i0 = 0; i0<queries.size() ;i0++)
	{
	    ...
	    if (i0 == sum_qb + q_batches[b])
	    {
            ...
            if (sum_sb > 0)
     	        for (int j = 0; j < K; j++)
		    neighbors_indices[i0 * K + j] += sum_sb;
	}
